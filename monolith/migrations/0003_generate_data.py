...
def generate_data(apps, schema_editor):
    import random
    from monolith.models import Brand, TypeProduct, OrderStatus, Product
    brands = 'Samsung', 'Apple', 'Xiaomi', 'Lenovo'
    product_types = 'Компьютер', 'Смартфон', 'Планшет'
    statuses = 'Оформлен', 'Оплачен', 'Получен',
    br_objects = Brand.objects.bulk_create(Brand(name=i) for i in brands)
    tp_objects = TypeProduct.objects.bulk_create(TypeProduct(name=j) for j in product_types)
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('monolith', '0002_rename_user_bucket_client'),
    ]
