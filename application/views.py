...
class BucketView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(BucketView, self).get_context_data()
        service = BucketService(user=self.request.user)
        object_list, pr_sum = service.get_products()
...
class BucketAddView(LoginRequiredMixin, TemplateView):
    def post(self, request, *args, **kwargs):
        ...
        Bucket.objects.create(
            product_id=post.get("product"), client_id=request.user.client.id
        )
        return HttpResponseRedirect(redirect_to=reverse("application:bucket_detail"))


class UserLogin(LoginView):
    template_name = "login.html"
    next_page = reverse_lazy("application:product_list")
...
class DeleteBucketProductFirst(LoginRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse("application:bucket_detail")
...
class OrderListView(LoginRequiredMixin, ListView):
    ...
    queryset = Order.objects.prefetch_related(
        "products_order", "products_order__product"
    )
...
class OrderCreateView(LoginRequiredMixin, TemplateView):
    def post(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(reverse("application:order_list"))
